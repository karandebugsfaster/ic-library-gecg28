...
export async function POST(request) {
  ...
  try {
    ...
    // Find student
    const user = await User.findOne({ enrollmentNumber: enrollmentNumber.trim().toUpperCase() }).session(session);
    ...
    await Notification.create([{
      ...
      message: `Manager assigned you "${book.title}". Due: ${dueDate.toDateString()}`,
      status: 'PENDING'
    }], { session });
